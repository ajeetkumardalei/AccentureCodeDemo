...
extension UIButton {
    func setImageFromAlmofireURL(strImgURL:String, isShowIndicator:Bool = false, placeholderimg:UIImage? = Placeholder.noImage, completionHandler:(_ success:Bool) -> () ) {
        ...
        self.af.setImage(for: .normal, url: url, cacheKey: nil, placeholderImage: Placeholder.noImage, serializer: nil, filter: nil, progress: nil, progressQueue: DispatchQueue.main) { (imgData) in
    ...
    func showSpinnerInMiddle(upto timeDuration:TimeInterval) {
        self.setImage(Placeholder.noImage, for: .normal)
